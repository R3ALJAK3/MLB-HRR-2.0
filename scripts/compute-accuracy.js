...
async function readGistFile(octokit, gistId, filename) {
  try {
    ...
    console.log(
      `  Could not read ${filename} from gist ${gistId}: ${e.message}`,
    );
    return null;
  }
}
...
function ladderFor(picks, results) {
  ...
  for (const t of THRESHOLDS) {
    ...
    const rate =
      played.length > 0 ? Math.round((hits / played.length) * 1000) / 10 : null;
...
function computeAccuracy(snapshot, boxscoreData) {
  ...
  const top10 = (snapshot.dailyTop10 || []).filter(
    (p) => p.name && p.name !== "Lineup TBD" && !p.isTBD,
  );
  const allPlayers = (snapshot.allPlayers || []).filter(
    (p) => p.name && p.name !== "Lineup TBD" && !p.isTBD,
  );
  ...
  const top10HitRate =
    top10Ladder.played > 0
      ? Math.round((v7Hits / top10Ladder.played) * 1000) / 10
      : null;
  const top10AvgDiff =
    v7Diffs.length > 0
      ? Math.round(
          (v7Diffs.reduce((a, b) => a + b, 0) / v7Diffs.length) * 100,
        ) / 100
      : null;
  const top10DNPRate =
    top10.length > 0
...
async function main() {
  ...
  console.log(
    `History gist: ${historyGistId}${sameGist ? " (same as live)" : ""}\n`,
  );

  const history = (await readGistFile(
    octokit,
    historyGistId,
    HISTORY_FILE,
  )) || { entries: [] };
  ...
  if (alreadyDone && !process.env.FORCE_RECOMPUTE) {
    ...
    if (!alreadyDone.top10ThresholdRates) {
      console.log(
        `  ${targetDate} already processed but missing threshold ladder — recomputing.`,
      );
    } else {
      console.log(
        `✓ Already processed ${targetDate} (HRR≥2 hit rate: ${alreadyDone.top10ThresholdRates[2]}%)`,
      );
  ...
  const snapshot = await loadYesterdaySnapshot(
    octokit,
    liveGistId,
    historyGistId,
    targetDate,
  ...
  if (!snapshot) {
    console.error(
      `✗ No snapshot found for ${targetDate}. Cannot compute accuracy.`,
    );
    console.error(
      `  Expected: snapshot-${targetDate}.json in history gist OR matching date in live gist.`,
    );
    process.exit(1);
  }
  ...
  if (boxscoreData.gamesFinal === 0) {
    console.error(
      `✗ No final games for ${targetDate}. Will retry on next run.`,
    );
    process.exit(2);
  }
  ...
  console.log(
    `  Played: ${dayAccuracy.top10Played}/${dayAccuracy.top10Total}  ·  DNP: ${dayAccuracy.top10DNPRate}%`,
  );
  ...
  for (const t of THRESHOLDS) {
    ...
    console.log(
      `    HRR ≥ ${t}:  ${rate != null ? rate + "%" : "—"}  (${hits}/${dayAccuracy.top10Played})${marker}`,
    );
  }
  console.log(
    `  v7-compat hit rate (actual ≥ round(proj)): ${dayAccuracy.top10HitRate}%`,
  );
  console.log(
    `  Avg diff:                                  ${dayAccuracy.top10AvgDiff >= 0 ? "+" : ""}${dayAccuracy.top10AvgDiff}`,
  );
